...
export class RefillService {
    ...
    async createRefill(userId: Types.ObjectId, data: CreateRefillDto): Promise<Refill[]> {
        const { orderId } = data;
        ...
        if (!orderCheck || orderCheck.length !== orderId.length) throw new BadRequestException("Order not found");
