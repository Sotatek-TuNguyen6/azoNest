...
export class PaypalService {
    ...
    async generateAccessToken(client_id: string, secret: string): Promise<string> {
        try {
    ...
    async createOrder(): Promise<string> {
        ...
        const accessToken = await this.generateAccessToken(deposit.value["client_id"], deposit.key);
    ...
    async checkOrder(orderID: string): Promise<PayPalOrder> {
        try {
            ...
            const response = await axios.get<PayPalOrder>(`${process.env.PAYPAL_BASE_URL}/v2/checkout/orders/${orderID}`, {
                headers: {
                    Authorization: `Bearer ${accessToken}`,
            ...
            if (response.status !== 200) {
                throw new BadRequestException('Đơn hàng không hợp lệ hoặc chưa thanh toán');
            }
    ...
    async callBack(userId: Types.ObjectId, orderId: string) {
        ...
        try {
            ...
            const amount = (Number(deposit.value["exchange"]) ?? 23000) * Number(orderDetail.amount.value);
