...
export class DepositService {
  ...
  async findOne(name: string, userId: Types.ObjectId) {
    ...
    const payee_account = name === "perfect_money" ? result.value["account_id"] : result.value["client_id"]

    const data = {
      "API_URL": name === "perfect_money" ? 'https://perfectmoney.is/api/step1.asp' : "",
  ...
  async callbackPaypal(data: PayPalWebhookDto) {
    ...
    try {
      const result = await this.depositModel.findOne({ name: "paypal" }).select("-key").session(session)
      switch (data.event_type) {
        case PayPalEventType.PAYMENT_CAPTURE_COMPLETED:
          ...
          const amount = result.value["exchange"] * Number(paymentDetails.amount.value)
  ...
  async callbackFpayment(data: FpaymentCallBack) {
    ...
    session.startTransaction();
    try {
      const { status, transaction_id, received, request_id } = data

      const invoice = await this.invoiceModel.findOne({ request_id, status: StatusInvoice.processing })
