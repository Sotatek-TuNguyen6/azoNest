    ...
    try {
      ...
      switch (channel) {
        case ChannelInvoice.FPAYMENT: {
          const deposit = await this.depositModel.findOne({ name: ChannelInvoice.FPAYMENT });
          ...
          if (!deposit.active) {
            throw new BadRequestException("Chức năng nạp tiền điện tử đang tạm bảo trì, vui lòng thử lại sau!");
          }

          const pendingInvoices = await this.invoiceModel.find({ user_id: userId, status: StatusInvoice.processing, type: 'fpayment' });

          if (pendingInvoices.length >= 3) {
            throw new BadRequestException("Bạn đã có 3 hóa đơn đang chờ xử lý, vui lòng chờ xử lý hoặc hủy bỏ hóa đơn cũ trước khi tạo hóa đơn mới");
          }
          ...
          const response = await axios.get('https://fpayment.co/api/AddInvoice.php', {
            params: {
          ...
          if (response.status !== 200) {
            throw new InternalServerErrorException('Server error, please contact admin!');
          }
